...
model1_string = " model {
    for (i in 1:length(Outcome)) {
        ...
        logit(p[i]) = b0 + b[1]*Pregnancies[i] + b[2]*Glucose[i] + b[3]*BMI[i] + b[4]*DiabetesPedigreeFunction[i] + b[5]*Age[i]
    }
...
data_jags = list(Outcome=finaldata.train$Outcome, Pregnancies=X1[,"Pregnancies"], Glucose=X1[,"Glucose"], BMI=X1[,"BMI"], DiabetesPedigreeFunction=X1[,"DiabetesPedigreeFunction"], Age=X1[,"Age"])
...
pm_Xb1 = pm_coef1["b0"] + X1[,c(1,2,3,4,5)] %*% pm_coef1[1:5] 
...
pm_Xb2 = pm_coef2["b0"] + X2[,c(1,2,3,4,5)] %*% pm_coef2[1:5] # Intercept + Design Matrix*Coefficients
...
# Intercept + Design Matrix*Coefficients
pm_Xb3 = pm_coef3["b0"] + X1[,c(1,2,3,4,5)] %*% pm_coef3[1:5] 
...
pm_Xb4 = pm_coef4["b0"] + X2[,c(1,2,3,4,5,6,7,8)] %*% pm_coef4[1:8] # Intercept + Design Matrix*Coefficients
